...
def transform_preds_to_waymo_format(pred_dicts, top_k_for_eval=-1, eval_second=8, joint_pred=False):
    ...
    for scene_idx, val in enumerate(scene2preds.items()):
        ...
        for obj_idx, cur_pred in enumerate(preds_per_scene):
            ...
            cur_pred['pred_scores'] = cur_pred['pred_scores'] + cur_pred['pred_scores'].argsort().argsort()
            if not joint_pred:
...
def waymo_evaluation(pred_dicts, top_k=-1, eval_second=8, num_modes_for_eval=6, joint_pred=False):

    pred_score, pred_trajectory, gt_infos, object_type_cnt_dict = transform_preds_to_waymo_format(
        pred_dicts, top_k_for_eval=top_k, eval_second=eval_second, joint_pred=joint_pred
    )
