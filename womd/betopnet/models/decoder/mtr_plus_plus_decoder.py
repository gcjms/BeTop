...
(Deprecated) Reproduced MTR++ Decoder: https://arxiv.org/abs/2306.17770
'''
...
class MTRPPDecoder(nn.Module):
    def __init__(self, in_channels, config):
        ...
        self.joint_decode = self.model_cfg.JOINT_DECODE
        ...
        if map_d_model != self.d_model:
            ...
            if self.joint_decode:
                self.map_rel_query_embed_mlps = nn.Linear(self.d_model, map_d_model)
        else:
    ...
    def build_dense_future_prediction_layers(self, hidden_dim, num_future_frames):
        ...
        self.dense_future_head = common_layers.build_mlps(
            # c_in=hidden_dim * 2,
            c_in=hidden_dim,
            mlp_channels=[hidden_dim, hidden_dim, num_future_frames * 8], ret_before_act=True
        )
    ...
    def build_transformer_decoder(self, in_channels, d_model, nhead, dropout=0.1, num_decoder_layers=1, use_local_attn=False):
        ...
        decoder_layers = nn.ModuleList([
            ...
            local_self_attention=True, rel_sa=True
        )
    ...
    def apply_dense_future_prediction(self, obj_feature, obj_mask, obj_pos):
        ...
        pred_dense_trajs_valid = self.dense_future_head(obj_feature_valid)
        pred_dense_trajs_valid = pred_dense_trajs_valid.view(pred_dense_trajs_valid.shape[0], self.num_future_frames, 8)
        ...
        ret_pred_dense_future_trajs = obj_feature.new_zeros(num_center_objects, num_objects, self.num_future_frames, 8)
    ...
    def get_motion_query(self, center_objects_type):
        if self.joint_decode:
            b, num_center, num_query, d = center_objects_type.shape
            intention_points = center_objects_type
            intention_query = position_encoding_utils.gen_sineembed_for_position(intention_points, hidden_dim=self.d_model)
            intention_query = self.intention_query_mlps(intention_query.view(-1, self.d_model)).view(b, num_center, num_query, self.d_model)
    ...
    def apply_cross_attention(self, kv_feature, kv_mask, kv_pos, query_content, query_embed, attention_layer,
                              ...
                              query_content_pre_mlp=None, query_embed_pre_mlp=None,map_rel_query_embed_mlp=None,

                              sa_index_pair=None, sa_query_batch_cnt=None, sa_key_batch_cnt=None,
                              sa_index_pair_batch=None, sa_rpe_distance=None, intention_mask_stack=None):
        """
        Args:
            kv_feature (B, N, C):
            kv_mask (B, N):
        ...
        if map_rel_query_embed_mlp is not None:
            sa_rpe_distance = map_rel_query_embed_mlp(sa_rpe_distance)
        ...
        if not use_local_attn:
            query_feature = attention_layer(
                ...
                sa_index_pair=sa_index_pair, 
                sa_query_batch_cnt=sa_query_batch_cnt, 
                sa_key_batch_cnt=sa_key_batch_cnt, 
                sa_index_pair_batch=sa_index_pair_batch,
                sa_mask=intention_mask_stack,
                rel_pos_sa=sa_rpe_distance,
            )  # (M, B, C)
        else:
    ...
    def make_global_rotation(self, dynamic_points, center_obj_pos):
        b, num_center, _ = center_obj_pos.shape
        num_query = dynamic_points.shape[1] // num_center
        future_t = dynamic_points.shape[-2]
        dynamic_points = dynamic_points.reshape(b, num_query, num_center, future_t, 2).permute(0, 2, 1, 3, 4)
        dynamic_points = dynamic_points.reshape(b, num_center, num_query*future_t, 2)
        rotate_intention = common_utils.poly_right_hand_rotations(dynamic_points, center_obj_pos[..., -1]) 
        rotate_intention = rotate_intention + center_obj_pos[:, :, None, :2]
        rotate_intention = rotate_intention.reshape(b, num_center, num_query, future_t, 2)
        rotate_intention = rotate_intention.permute(0, 2, 1, 3, 4).reshape(b, num_query * num_center, future_t,  2)
        return rotate_intention
    
    def apply_global_motion_query(self, intention_points, intention_query, center_obj_pos, center_mask=None, num_of_neighbors=16):
        '''
        intention_points: [b, 8, 64, 2]
        center_obj_pos:[b, 8, 3]
        '''
        b, num_center, num_query, d_model = intention_query.shape
        intention_mask = intention_points.sum(-1) != 0

        rotate_intention = common_utils.poly_right_hand_rotations(intention_points, center_obj_pos[..., -1]) 
        rotate_intention = rotate_intention + center_obj_pos[:, :, None, :2]

        intention_mask = intention_mask.permute(0, 2, 1).reshape(b, num_query * num_center)
        rotate_intention = rotate_intention.permute(0, 2, 1, 3).reshape(b, num_query * num_center, 2)
        full_center_obj_pos = center_obj_pos[:, None, :, :].repeat(1, num_query, 1, 1).reshape(b, num_query * num_center, 3)

        rotate_intention = torch.cat([rotate_intention, full_center_obj_pos[..., -1, None]], dim=-1)

        intention_query = intention_query.permute(0, 2, 1, 3)
        intention_query = intention_query.contiguous().view(b, num_query * num_center, -1)
        intention_query = intention_query.view(-1, d_model)
        intention_mask_stack = intention_mask.view(-1)
        rotate_intention_pos_stack_full = rotate_intention.view(-1, 3)
        global_pos_stack_full = full_center_obj_pos.view(-1, 3)
        batch_idxs_full = torch.arange(b).type_as(intention_query)[:, None].repeat(1, num_query * num_center).view(-1).int()  # (batch_size * N)

        # filter invalid elements
        # x_stack = intention_query[intention_mask_stack]
        x_pos_stack = rotate_intention_pos_stack_full[intention_mask_stack]
        global_pos_stack = global_pos_stack_full[intention_mask_stack]
        batch_idxs = batch_idxs_full[intention_mask_stack]

        # knn
        batch_offsets = common_utils.get_batch_offsets(batch_idxs=batch_idxs, bs=b).int()  # (batch_size + 1)
        ...
        query_attn_pos = grouping_operation(
            ...
            # (num_valid_elems, K, 3)
        rel_attn_pos = query_attn_pos - global_pos_stack[:, None, :]
        heading =  global_pos_stack[..., 2]
        ...
        pos_embedding = position_encoding_utils.gen_sineembed_for_position(rel_pos[:, :, :4], hidden_dim=d_model)

        return pos_embedding, index_pair, batch_idxs, batch_cnt, intention_query, intention_mask_stack
    
    def update_anchors(self, pred_waypoints):
        updated_anchors = pred_waypoints[..., -1, :2]

    def apply_transformer_decoder(self, center_objects_feature, center_objects_type, obj_feature, obj_mask, obj_pos, map_feature, map_mask, map_pos, center_obj_pos,
        center_mask=None):
        intention_query, intention_points = self.get_motion_query(center_objects_type)
        #b, num_center, num_query self.d_model

        if self.joint_decode:
            b, num_center, num_query_single, d_model = intention_query.shape
            pos_embedding, index_pair, batch_idxs, batch_cnt, intention_query, intention_mask_stack = self.apply_global_motion_query(
                intention_points, intention_query, center_obj_pos, center_mask, num_of_neighbors=16
            )
            query_content = torch.zeros((num_query_single *num_center, b, d_model)).to(intention_query.device)
            self.forward_ret_dict['intention_points'] = intention_points.view(b * num_center, num_query_single, 2) #b, num_center, num_query, 2
        else:
            pos_embedding, index_pair, batch_idxs, batch_cnt, intention_query, intention_mask_stack = None, None, None, None, None, None
        ...
        if self.joint_decode:
            center_objects_feature = center_objects_feature[:, None, :, :].repeat(1, num_query_single, 1, 1).reshape(b, num_query_single*num_center, -1)
        ...
        base_map_idxs = None
        if self.joint_decode:
            intention_points = intention_points.permute(2, 1, 0, 3).reshape(num_query, b, 2)
        ...
        for layer_idx in range(self.num_decoder_layers):
            ...
            obj_query_feature = self.apply_cross_attention(
                ...
                layer_idx=layer_idx,
                # for rel self-attn:
                sa_index_pair=index_pair, sa_query_batch_cnt=batch_cnt, sa_key_batch_cnt=batch_cnt,
                sa_index_pair_batch=batch_idxs, sa_rpe_distance=pos_embedding, 
                intention_mask_stack=intention_mask_stack
            ) 

            # query map feature
            collected_idxs, base_map_idxs = self.apply_dynamic_map_collection(
                map_pos=map_pos, map_mask=map_mask,
                pred_waypoints=self.make_global_rotation(pred_waypoints, center_obj_pos) if self.joint_decode else pred_waypoints,
            ...
            map_query_feature = self.apply_cross_attention(
                ...
                map_rel_query_embed_mlp=self.map_rel_query_embed_mlps,
                # for rel self-attn:
                sa_index_pair=index_pair, sa_query_batch_cnt=batch_cnt, sa_key_batch_cnt=batch_cnt,
                sa_index_pair_batch=batch_idxs, sa_rpe_distance=pos_embedding, 
                intention_mask_stack=intention_mask_stack
            ...
            if self.joint_decode:
                out_pred_scores = pred_scores.view(b, num_query_single, num_center, -1)
                out_pred_trajs = pred_trajs.view(b, num_query_single, num_center, 80, -1)
                b, num_q, num_c, t, d = out_pred_trajs.shape
                out_pred_trajs = out_pred_trajs.permute(0, 2, 1, 3, 4).reshape(b * num_c, num_q, t, d)
                out_pred_scores = out_pred_scores.permute(0, 2, 1, 3).reshape(b * num_c, num_q)
                pred_list.append([out_pred_scores, out_pred_trajs])
            else:
    ...
    def get_decoder_loss(self, tb_pre_tag=''):
        ...
        assert center_gt_trajs.shape[-1] == 5
        ...
        if self.joint_decode:
            center_mask = self.forward_ret_dict['center_mask'].bool()
            # print(center_gt_trajs.shape, center_mask.shape)
            center_gt_trajs = center_gt_trajs[center_mask]
            center_gt_trajs_mask = center_gt_trajs_mask[center_mask]
            center_gt_final_valid_idx = center_gt_final_valid_idx[center_mask]
            intention_points = intention_points[center_mask]
        ...
        for layer_idx in range(self.num_decoder_layers):
            ...
            if self.joint_decode:
                pred_scores, pred_trajs = pred_scores[center_mask], pred_trajs[center_mask]
            ...
            loss_reg_vel = F.l1_loss(pred_vel, center_gt_trajs[:, :, 3:5], reduction='none')
            ...
            # print(self.forward_ret_dict['center_objects_type_char'].shape)
            # print(self.forward_ret_dict['center_objects_type_char'][center_mask.cpu().numpy().astype(np.bool)])
            ...
            if layer_idx + 1 == self.num_decoder_layers:
                layer_tb_dict_ade = motion_utils.get_ade_of_each_category(
                    ...
                    object_types=self.forward_ret_dict['center_objects_type_char'][center_mask.cpu().numpy().astype(np.bool)] if self.joint_decode else self.forward_ret_dict['center_objects_type'],
    ...
    def fut_traj_rotate_along_z(self, obj_trajs, curr_pos):
        """
        traj: [batch, t, 9]
        ...
        """
        curr_xy, curr_head = curr_pos[..., :2] , curr_pos[..., -1]
        traj_xy = obj_trajs[..., :2] - curr_xy[..., None, :2]
      
        rotated_xy = common_utils.poly_left_hand_rotations(traj_xy, curr_head)
        rotated_vxy = common_utils.poly_left_hand_rotations(obj_trajs[..., [3, 4]], curr_head)

        obj_trajs[..., :2] = rotated_xy
        obj_trajs[..., [3, 4]] = rotated_vxy
        full_heading = obj_trajs[..., 2].clone()
        obj_trajs[..., 2] = common_utils.wrap_to_pi(full_heading - curr_head[..., None])
    ...
    def get_dense_future_prediction_loss(self, tb_pre_tag='', tb_dict=None, disp_dict=None):
        ...
        obj_traj_curr_state = self.forward_ret_dict['obj_pos'].cuda()
        obj_trajs_future_state = self.fut_traj_rotate_along_z(obj_trajs_future_state, obj_traj_curr_state)
        ...
        assert pred_dense_trajs.shape[-1] == 8
        assert obj_trajs_future_state.shape[-1] == 5 #[x, y, theta, vx, vy]

        pred_dense_trajs_gmm, pred_dense_heading, pred_dense_trajs_vel = pred_dense_trajs[:, :, :, :5], pred_dense_trajs[..., 5], pred_dense_trajs[:, :, :, 6:8]

        loss_reg_head = F.l1_loss(pred_dense_heading, obj_trajs_future_state[..., 2], reduction='none')
        loss_reg_head = (loss_reg_head * obj_trajs_future_mask[:, :, :]).sum(dim=-1)

        loss_reg_vel = F.l1_loss(pred_dense_trajs_vel, obj_trajs_future_state[:, :, :, [3, 4]], reduction='none')
        loss_reg_vel = (loss_reg_vel * obj_trajs_future_mask[:, :, :, None]).sum(dim=-1).sum(dim=-1)

        # print(loss_reg_head.shape, loss_reg_vel.shape, pred_dense_heading.shape, obj_trajs_future_mask.shape, )
        ...
        loss_reg = loss_reg_vel + loss_reg_gmm + loss_reg_head
    ...
    def forward(self, batch_dict):
        ...
        center_obj_pos = batch_dict['center_obj_pos']
        num_center_objects, num_objects, _ = obj_feature.shape
        ...
        # decoder layers
        if self.joint_decode:
            center_mask = input_dict['center_obj_mask']
            # center_mask = center_mask.reshape(-1, 8)
        else:
            center_mask = None
        ...
        if not self.training:
            ...
            self.forward_ret_dict['center_objects_type_char'] = input_dict['center_objects_type_char']
            self.forward_ret_dict['obj_pos'] = batch_dict['obj_pos']
            if self.joint_decode:
                self.forward_ret_dict['center_mask'] = center_mask
